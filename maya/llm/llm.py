...
class LLMClient:
    ...
    async def generate(
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                print("  Please check your API key configuration:", file=sys.stderr, flush=True)
                print(f"    - Set {provider_upper}_API_KEY environment variable", file=sys.stderr, flush=True)
                print("    - Or create ~/.maya/config.json with valid credentials", file=sys.stderr, flush=True)
                print(
                    "  Supported providers: OpenAI, Anthropic, Google, Azure, Vertex AI, Ollama, etc.",
                    file=sys.stderr,
                    flush=True,
                )
                sys.exit(1)
                ...
                print(f"  Error: {exc}", file=sys.stderr, flush=True)
                print("  Your API key does not have access to this model.", file=sys.stderr, flush=True)
                print(
                    f"  Try a different {provider} model or check your plan/subscription.", file=sys.stderr, flush=True
                )
                sys.exit(1)
                ...
                if attempt >= self.config.max_retries:
                    ...
                    print(f"  Error: {exc}", file=sys.stderr, flush=True)
                    print("  You've hit your API rate limit. Try again later.", file=sys.stderr, flush=True)
                    sys.exit(1)
                wait_time = delay * (2**attempt)
                retry_msg = (
                    f"  [WARN] Rate limit hit, retrying in {wait_time}s... "
                    f"(attempt {attempt + 1}/{self.config.max_retries + 1})"
                )
                print(retry_msg, file=sys.stderr, flush=True)
                ...
                print(f"  Error: {exc}", file=sys.stderr, flush=True)
                print("  Check that your model name is correct.", file=sys.stderr, flush=True)
                print(f"  Format: provider/model-name (e.g., {provider}/model-name)", file=sys.stderr, flush=True)
                print(
                    "  Examples: openai/gpt-4o-mini, anthropic/claude-sonnet-4, google/gemini-1.5-pro",
                    file=sys.stderr,
                    flush=True,
                )
                sys.exit(1)
                ...
                if attempt >= self.config.max_retries:
                    ...
                    print(f"  Error: {exc}", file=sys.stderr, flush=True)
                    print("  The LLM provider is having issues. Try again later.", file=sys.stderr, flush=True)
                    sys.exit(1)
                retry_msg = (
                    f"  [WARN] API error, retrying in {delay}s... (attempt {attempt + 1}/{self.config.max_retries + 1})"
                )
                print(retry_msg, file=sys.stderr, flush=True)
                ...
                if attempt >= self.config.max_retries:
                    ...
                    print("  This might be a network issue or unsupported model.", file=sys.stderr, flush=True)
    ...
    async def validate(self) -> bool:
        ...
        try:
            ...
            if response.content:
                print("LLM connection valid", file=sys.stderr, flush=True)
                return True
            return False
        except Exception:
